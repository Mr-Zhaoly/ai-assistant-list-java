...
import com.zly.model.vo.BusinessAgentVO;
...
public class BusinessController {
                ...
                .map(output -> {
                    ...
                    if (output instanceof StreamingOutput streamingOutput) {
                        if (streamingOutput.getOutputType() == OutputType.AGENT_MODEL_STREAMING) {
                            BusinessAgentVO agentVO = BusinessAgentVO.builder()
                                    .code(AgentCodeEnum.SUCCESS.getCode())
                                    .node(node)
                                    .chunk(streamingOutput.message().getText())
                                    .build();
                            return "data:" + JSON.toJSONString(agentVO) + "\n\n";
                        }
    ...
    public Flux<String> feedback(@RequestBody FeedbackRequest request) throws GraphRunnerException {
        ...
        if (metadata == null) {
            return Flux.just("data:" + JSON.toJSONString(Map.of("type", "error", "content", "No pending interruption found for this session.")) + "\n\n");
        }
